...
ZeroDB is agnostic to which encryption algorithm use.
...
Using deterministic or order-preserving encryption visible to the server could be detrimental for security of the database in certain cases~\cite{cryptdb-hacked}.
However, even traversal of B-Trees leaves a possibility for attacher to collect statistics of accesses and infer data in the database~\cite{access-pattern-attack}.
We provide security analysis and example of number of queries necessary for such an attack.

\begin{figure}
	\begin{center}
        \includegraphics[width=0.7\columnwidth]{surname-attack.pdf}
	\end{center}
    \caption{Fraction of a database of last names leaked to a passive observer depending on number of queries made to the database}
	\label{fig:surname-attack}
\end{figure}

\subsection{Threat model}

We adopt threat model similar to one used for security analysis of CipherBase~\cite{cipherbase}.
We introduce a \emph{strong adversary}, who has unlimited observational power, can view contents of memory and disk at every moment of time.
...
Therefore, a weak adversary doesn't learn anything except for database size (including number of fields, etc), even during queries.
So, ZeroDB provides \emph{semantic security} against a weak adversary.

Strong adversary, on the other hand, is able to observe access patterns and collect statistics of those over time.
