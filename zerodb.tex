...
In this process, the server which observes a search pattern can infer which which level of the tree buckets belong to,
however it cannot figure out ordering of elements as it cannot see which link from the bucket was used.
...
\subsection{Complex queries (multi-keyword search, multiple conditions)}  % XXX
...
Full-text search with keywords $t_1, t_2,\ldots, t_n$ requires to order results by net scores of documents relative to these keywords.
One rarely needs all the matching documents at once, and the result is almost always sorted by the score.
That makes it possible to do in incremental, pre-ordered manner, even though the score depends on the query.

Okapi BM25~\cite{wiki:OkapiBM25} is considered to be a state-of-art ranking function.
However, when it comes to scalability, it doesn't work well because term frequency TF in its definition depends on both length of a document
and mean length of a document in a database.
It makes it impossible to pre-compute score even for one keyword $t$ without necessity to fetch individual numbers of occurrences of $t$
in all documents where it appears.
Instead, we use Lucene's practical scoring function~\cite{lucene-practical-scoring}.

In a simple form, the score is determined as following.
For a term $t$ and document $D$:
$$TF(t, D) = \sqrt{f(t, D)},$$
where $f(t, D)$~--- number of occurrences of $t$ in $D$.
$$IDF(t) = 1 + \ln\left( \frac{N_{docs}}{N_{docs}(t) + 1} \right),$$
where $N_{docs}$ is total number of text documents indexed, $N_{docs}(t)$ is number of documents which contain term $t$.
Values of $N_{docs}$ and $N_{docs}(t)$ are stored in encrypted form and updated when new documents are added.
...
% XXX let's mention Pailier/ElGamal separately for Length() property
Score of a document for one term $t$ will be:
$$s(t, D) = \frac{TF(t, D) \cdot IDF(t)^2}{\sqrt{|D|}},$$
where $|D|$ is total number of unique terms in document $D$.
$TF(t,D)/\sqrt{|D|}$ is calculated at indexing time, $IDF$ is calculated at query time since IDF doesn't depend on $D$.
This, for one-keyword queries documents are already pre-ordered by relevancy.

Total document weight is defined as:
$$s(D) = \frac{\sum_{i=0}^n s(t_i, D)}{\sqrt{\sum IDF(t_i)^2}}$$.
This results into having to sum up scores $TF(t,D)/\sqrt{|D|}$ which document references are pre-ordered by (in a descending order)
with weights of $IDF^2/\sqrt{\sum IDF}$ calculated at query time.

In order to produce docids sorted by $s$ incrementally, we first tuples $(s_j(t_i, D), docid_j)$ with highest values of $s$ for all the query terms $t_i$.
Then we calculate possible ranges of document scores, given our knowledge of highest known scores for terms $t_i$ and scores for particular $docids$ which we've read.
...
If known documents which haven't yet been returned don't overlap by their range of possible scores when sorted by $\min(s)$,
we are ok to grab documents from the cache.
If ranges of possible scores overlap, we don't have enough info and we read more document scores for the keyword where the difference between scores for different documents is currently the highest.
...
% XXX TO REMOVE: we'll not do this. Too insecure.
% Concept of tree of keys is still useful, will copy to the sharing section
