...
First, we consider search queries which result just in tree traversal.
Those could be limit orders for queries which don't involve pulling a large subset of the index to the client.
Let's assume for simplicity that all we have in a database index is a B-Tree of surnames of customers.
An active adversary is able to acquire statistics of accesses of the index leaf nodes when searches of, say, $10$ people with a given last name are performed.
However, the amount of statistics should be large enough to be able to distinguish different records in the database.
If we assume that probability of a given surname to appear in a given query is the same as in the U.S. Census 2000 data~\cite{us-census-surnames}
and that keywords are distributed across queries according to Poisson distribution,
an adversary needs to observe two keywords at least $N_1$ and $N_2$ times such that $|N_1 - N_2| > \sqrt{N_1 + N_2}$.
Fraction of last names which can be revealed by a strong adversary from a query depending in number of queries is shown on~\figref{fig:surname-attack}.
For example, surnames in $10\%$ of new queries can be inferred by a strong adversary after observing a million queries (which still doesn't reveal surnames not touched by queries).
Unlike CipherBase~\cite{cipherbase}, our approach doesn't leak information about order of the elements.
By observing access patterns, the server can only figure out which buckets are leaf nodes and which are branch nodes.

In cases when we need to prefetch a large chunk of a B-Tree~(Sec.~\ref{sec:prefetch}), we immediately reveal information about the size of the sub-dataset to the server.
Of course, this is not desirable.
In order to make it more secure, we can download adjacent records in the same query so that the downloaded size is always approximately constant and equal to that of the largest sub-dataset.
This way our security guarantees are similar to those of just tree traversal.
In order for this to work we need to unfold nested trees into one tree~(Sec.~\ref{sec:unfold-trees}).

Security guarantees of server-side set intersections~(Sec.~\ref{sec:server-intersection}) are weaker.
One query leaks sum of sizes of datasets involved in the query and how many elements satisfy any $c$ conditions from the query where $1\le c\le n$, where $n$~--- number of conditions in the query.
...
They allow to hide access patterns by writing back data after reading it.
Several algorithms are available, and most promising ones are Burst ORAM~\cite{burst-oram}
and oblivious data structures~\cite{ods-wang-2014}.
Still, these require to write back to the database about 20 times the amount of data read,
but these numbers improve every year.
It is also possible that hiding access patterns in multi-cloud or fully decentralized arrangement has quite good performance properties~\cite{oram-multicloud},
...
$$h \approx \frac{\ln s_c - \ln s_r}{\ln s_b - \ln s_r}.$$
...
In ZeroDB prototype, the typical size of a record in the bucket is $s_r = 30$ bytes,
compression rate with zlib $c=4$.
For a typical network bandwidth of $b=10^6$ (1 MB/s) and roundtrip time $\tau=0.05~\mbox{s}$, the optimal bucket size is $s_b=110kB$.
For a typical cache size of $1$~MB (or $250$~kB when compressed) and index size of $50$~GB,
one query using this index on average takes $t=0.3$~s and client-server exchange of $36$~kB of data (without caching that would take $0.7$~s and exchanging $80$~kB of data).
